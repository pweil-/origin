...
//					through to the host_be.  Driven by the termination type of the backend
//
...
func hasValidTLS(route *routeapi.Route) bool {
	return &route.TLS != nil && len(route.TLS.Termination) > 0 && len(route.TLS.Certificate) > 0
}
