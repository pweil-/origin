...
// Create a PodSecurityContext based on the given constraints.  If a settingis already set
// on the PodSecurityContext it will not be changed.  Validate should be used after the context
// is created to ensureit complies with the required restrictions.
//
...
func (s *simpleProvider) CreatePodSecurityContext(pod *api.Pod) (*api.PodSecurityContext, error) {
	...
	if sc.FSGroup == nil {
		fsGroup, err := s.fsGroupStrategy.Generate(pod)
		if err != nil {
			return nil, err
		}
		// strategy is not required to provide a value here so the array may be empty (RunAsAny)
		if len(fsGroup) > 0 {
			sc.FSGroup = &fsGroup[0]
		}
	}
